...
const run = async () => {
  try {
    ...
    // Find requests with matchedDonor present
    ...
    for (const r of requests) {
      try {
        // Try to resolve donorId from snapshot
        let donorId = null
        try {
          if (r.matchedDonor && r.matchedDonor.raw && r.matchedDonor.raw._resolvedDonor && r.matchedDonor.raw._resolvedDonor.id) {
            donorId = r.matchedDonor.raw._resolvedDonor.id
          }
        ...
        if (!donorId && r.matchedDonor) {
          // best-effort lookup by name + blood
